...
DBS_STANDARD_TAGS = dict([
    ...
    ("dbs-costcenter", ("dbs-costcenter", "N/A")),
...
def retrieve_default_accountwide_dbs_tag_values_from_ssm_parameters():
    ...
    for param in params_resp['Parameters']:
        # Collect deprecated 'project' and 'projecttype' to concat into 'costcenter' tag
        if SSM_PARAM_PREFIX+'project' == param['Name']:
            tmp_proj = param['Value']
        if SSM_PARAM_PREFIX+'projecttype' == param['Name']:
            tmp_projtype = param['Value']

        parameters.setdefault(param['Name'], param['Value'])
    parameters.setdefault("dbs-costcenter", tmp_proj + ":" + tmp_projtype)
    return parameters
...
def process_organization_units_level_for_tag_updates(org_units, parameters):
    for org_unit in org_units['OrganizationalUnits']:
        add_missing_organization_resource_tags(org_unit['Id'], parameters, org_unit['Name'])
        # org_unit['Name']
...
def add_missing_organization_resource_tags(resource_id, parameters, resource_name=""):
    try:
        ...
        for tag in tags:
            if tag["Key"] == "Name" and tag["Value"] == "TODO":
                Tagger.org_client.tag_resource(ResourceId=resource_id, Tags=[{'Key': "Name", 'Value': resource_name}])
                LOGGER.info("Tagged %s with %s=%s", resource_id, "Name", resource_name)
            else:
                continue
        ...
        for dbs_tag, tag_config in DBS_STANDARD_TAGS.items():
            if not any(tag['Key'] == dbs_tag for tag in tags):
                ...
                if is_product:
                    ...
                    Tagger.service_catalog_client.update_portfolio(Id=id,
                                                                   AddTags=[{'Key': dbs_tag, 'Value': use_value}])
