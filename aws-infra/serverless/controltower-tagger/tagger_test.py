...
class TestTagger(unittest.TestCase):
    ...
    def test_get_params(self):
        ...
        # assert
        TestTagger.describe_params_mock.assert_called_once_with(ParameterFilters=
                                                                [{'Key': 'Name', 'Option': 'Contains',
                                                                  'Values': [SSM_PARAM_PREFIX]}]
                                                                )
    ...
    def test_tag_root(self):
        ...
        for i in range(len(TestTagger.ssm_param_names)):
            TestTagger.tag_org_resource_mock.assert_any_call(ResourceId=TestTagger.root_id, Tags=[
                {'Key': 'dbs-' + TestTagger.ssm_param_names[i].split("/")[-1], 'Value': TestTagger.tag_values[i]}])
    ...
    def test_tag_OUs(self):
        ...
        handler(TestTagger.acct_created_event, TestTagger.lambda_context)
        # assert
        self.assertEqual(TestTagger.list_orgs_for_parent_mock.call_count,
                         len(TestTagger.org_units['OrganizationalUnits']) + 1)
        for i in range(len(TestTagger.org_units)):
            TestTagger.list_org_tags_mock.assert_any_call(
                ResourceId=TestTagger.org_units['OrganizationalUnits'][i]['Id'])
            for j in range(len(TestTagger.ssm_param_names)):
                TestTagger.tag_org_resource_mock.assert_any_call(
                    ResourceId=TestTagger.org_units['OrganizationalUnits'][i]['Id'], Tags=[
                        {'Key': 'dbs-' + TestTagger.ssm_param_names[j].split("/")[-1],
                         'Value': TestTagger.tag_values[j]}])
    ...
    def test_tag_accounts(self):
        ...
        for i in range(len(TestTagger.accounts)):
            ...
            for j in range(len(TestTagger.ssm_param_names)):
                TestTagger.tag_org_resource_mock.assert_any_call(ResourceId=TestTagger.accounts['Accounts'][i]['Id'],
                                                                 Tags=[{'Key': 'dbs-' +
                                                                               TestTagger.ssm_param_names[j].split("/")[
                                                                                   -1],
                                                                        'Value': TestTagger.tag_values[j]}])
    ...
    def test_tag_policies(self):
        ...
        for i in range(len(taggedPolicies)):
            ...
            for j in range(len(TestTagger.ssm_param_names)):
                TestTagger.tag_org_resource_mock.assert_any_call(ResourceId=taggedPolicies[i]['Id'], Tags=[
                    {'Key': 'dbs-' + TestTagger.ssm_param_names[j].split("/")[-1], 'Value': TestTagger.tag_values[j]}])
    ...
    def test_tag_products(self):
        ...
        for i in range(len(TestTagger.products)):
            ...
            for j in range(len(TestTagger.ssm_param_names)):
                TestTagger.update_product_mock.assert_any_call(Id=product_id, AddTags=[
                    {'Key': 'dbs-' + TestTagger.ssm_param_names[j].split("/")[-1], 'Value': TestTagger.tag_values[j]}])
    ...
    def test_tag_portfolios(self):
        ...
        for i in range(len(TestTagger.portfolios)):
            ...
            for j in range(len(TestTagger.ssm_param_names)):
                TestTagger.update_portfolio_mock.assert_any_call(Id=portfolio_id, AddTags=[
                    {'Key': 'dbs-' + TestTagger.ssm_param_names[j].split("/")[-1], 'Value': TestTagger.tag_values[j]}])
